...
4. `docs/24-first-pr.md` (your initial deliverable).
...
- P1-T02: Activate `ci.yml` (rename from `.tpl`); verify matrix builds green.
- P1-T03: Activate `security.yml`; verify cargo-audit + dependency-review + trufflehog + SLSA attestations.

**Branch:** `tl/bootstrap-P1-T00-T01-T02-T03`. One PR for the bootstrap; subsequent PRs for any further WS-A work.
...
- CI workflow files must enforce the hard-rules-scan from the templates (refuse offensive-target patterns; refuse Apollo deps in Artemis crates).
- ADR-0001 (Rust core), ADR-0002 (OTLP), ADR-0003 (aya), ADR-0008 (licensing deferred) all stand; do not introduce contradictions.
